	...
	public void run() {
		while (true) {
			try {
				...
				long countCoordinator = List.processes.stream().filter(process -> process.isCoordinator()).count();
				if(countCoordinator == 0 && List.processes.isEmpty()) {
					firstProcessList = true;
				}
				
				if (List.processes.stream().filter(process -> process.getIdProcess() == processId).count() > 0) {
					while (true) {
						processId = random.nextInt(Integer.MAX_VALUE);

						if (List.processes.stream().filter(process -> process.getIdProcess() == processId)
								.count() > 0) {
							continue;
						} else {
							break;
